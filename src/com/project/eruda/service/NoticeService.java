	...
	public static void list(String title, int idx) {
			...
			if (sel.equals("0")) {
				...
				if( idx-10 < 0) {
					...
					idx -= 10;
					NoticeService.list("<  공지사항  >", idx);
				}
			} else if (sel.equals("3")) {
				
				if(idx+10 < Data.noticeList.size() ) {
					idx += 10;
					NoticeService.list("<  공지사항  >", idx);
				}
				else 
		...
		/**
		 * <h1>열람하고 싶은 공지사항 글을 선택해 본문을 열람하기 위한 메소드</h1>
		 * 입력받은 글 번호에 해당되는 글이 없을 땐 다시 글 번호 입력받음<br><br>
		 * 0 선택 시 > {@link NoticeService#list(String, int)} 호출해서 글목록 출력<br>
		 * 1 선택 시 > {@link NoticeService#edit(String)} 호출해서 글 수정<br>
		 * 2 선택 시 > {@link NoticeService#delete(String)} 호출해서 글 삭제<br>
		 * 
		 * 
		...
		private static void read(String num) {
			...
			for (Notice n : Data.noticeList) {
				
				if (Integer.parseInt(num) > Data.noticeList.size()) {
					System.out.println("\n\t==================================================================\n");
					System.out.println("\t해당 번호의 글이 존재하지 않습니다. 다시 입력해주세요\n");
					System.out.print("\t▶ 열람하고 싶은 글 번호를 입력해주세요 : ");
					String sel = scan.nextLine();
					NoticeService.read(sel); // ** 재귀 쓰지말기 > 함수 호출스텍 꼬임
					break; // for문 안에 있는 함수는 조심
				}
				
				if (num.equals(n.getIndex())) {
					...
					if (sel.equals("0")) {
						...
						NoticeService.edit(n.getIndex());
						break;
					} else if (sel.equals("2")) { //
						NoticeService.delete(n.getIndex());
		...
		/**
		 * <h1>새로운 공지사항을 추가하기 위한 메소드</h1>
		 * 제목과 본문 내용을 입력받아 데이터에 추가한다.<br>
		 * 현재 로그인 유저가 관리자 계정일 때만 글 작성이 가능하다.<br>
		 * 
		 */
		public static void add() {
			...
			if ( Data.noticeList.size() == 0)
				...
				no = Data.noticeList.stream().mapToInt(n -> Integer.parseInt(n.getIndex())).max().getAsInt() + 1 + "";
			...
			String id = Main.currentAdmin.getName();

			date += now;
			Notice n = new Notice(no, date, id, title, text);
			Data.noticeList.add(n);// 회원 추가하기
			Data.saveNoticeList();
			NoticeService.list("<  공지사항  >", 0);
		 ...
		 * 
		 * {@link NoticeService#checkUser(int, String)} 메소드를 통해 선택한 글의 글쓴이와 현재 로그인 유저의 아이디가 같은지 확인하고 비밀번호를 입력받는다.<br><br>
		 * 관리자 계정(admin 계정)일 때만 수정이 가능하다.<br><br>
		 * {@link NoticeService#checkPW(String)}메소드를 통해 비밀번호 비교를 하고 비밀번호가 일치하면 수정을 원하는 글의 본문 내용을 새로 입력받아서 다시 저장한다.<br>
		 * 		-> 비밀번호가 틀릴 시 수정이 불가하다.
		 * 
		 * 
		 *   
			...
			if (sel.equals("0")) {
				...
				if (Main.currentAdmin == null) {
					...
					scan.nextLine();
						...
						if (NoticeService.checkUser(curIdx, "수정") == true) {
		...
		/**
		 * <h1>원하는 공지사항 글을 삭제하기 위한 메소드</h1>
		 * 관리자 계정(admin 계정)일 때만 삭제가 가능하다.<br><br>
		 * {@link NoticeService#checkUser(int, String)}메소드를 통해 선택한 글의 글쓴이와 현재 로그인 유저의 아이디가 같은지 확인하고 비밀번호를 입력받는다.<br>
			...
			if (sel.equals("0")) {
				...
				if (Main.currentAdmin == null) {
					...
					scan.nextLine();
						...
						if (NoticeService.checkUser(curIdx, "삭제") == true) {
							for (Notice n : Data.noticeList) {
				
								if (n.getIndex().equals(num)) {
									Data.noticeList.remove(n); 
									Data.saveNoticeList();
									break;
								}
							...
							for (Notice n : Data.noticeList) {
								...
								int idx = Integer.parseInt(n.getIndex());
		...
		/**
		 * <h1>현재 로그인 유저와 선택한 공지사항의 작성자 ID를 비교하는 메소드</h1>
		...
		private static boolean checkID(int num) {
			...
			Notice n = Data.noticeList.get(num-1);
			
			if (Main.currentAdmin.getName().equals(n.getId())) {
				return true;			
			} else {
				return false;
			}
		...
		private static boolean checkPW(String pw) {
			...
			if (Main.currentAdmin.getPw().equals(pw)) {
				return true;			
			} else {
				return false;
			}
		...
		private static boolean checkUser(int curIdx, String mode) {
			...
			Scanner scan = new Scanner(System.in);
			if (NoticeService.checkID(curIdx) == true) {
				System.out.println("\n\t==================================================================\n");
				System.out.print("\t▶ 비밀번호를 입력해주세요 :");
				String pw = scan.nextLine();
				
				if (NoticeService.checkPW(pw) == false ) {
					System.out.println("\n\t==================================================================\n");
